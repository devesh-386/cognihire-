...
decide, and it is not trained on hiring outcomes. That restriction is what makes the rest of this section a measurement problem instead of a prediction problem about people. Table~\ref{tab:ml-paradigms} states the paradigms and where
each is admissible.
...
exploration means asking a real candidate a question you already believe is poor \\
\bottomrule
\end{tabular}
\end{table*}
...
absent from its training data. Two quantities are fitted inside each fold, on training rows only: the IDF corpus statistics and the feature scaler. Fitting either over the pooled corpus would leak the test fold's term distribution into training. Identical fold
...
receive standardised features and tree ensembles receive raw features, since standardising a scale-invariant model would compare preprocessing, not algorithms.
...
$p = 6.5 \times 10^{-6}$), so the models are not interchangeable. The post-hoc picture, though, is what governs a ranking claim. Comparing the leader against
...
Although a study reporting only the ranked table would license the claim that the MLP is the best model for résumé--job fit, the significance analysis does not support it.
...
shown to detect a difference when one exists. We therefore constructed a synthetic control with a pre-specified generative process: twelve features, eight carrying linear weights, one interaction term, one quadratic term, four weightless, and a 7\% label-flip rate imposing a Bayes-optimal accuracy of 0.925. We ran it through the identical protocol. There the same nine models
...
reporting something. The comparison licenses one inference. When learnable signal is present, the protocol resolves it clearly; on the résumé task it does not. The null is a property of the task, not a limitation of the study.
...
performance comes from. A probe given only the résumé embedding cannot be matching, since it never sees the thing being matched against. It reaches 0.660 AUROC, 87\% of the best model's score and statistically indistinguishable
from a single cosine-similarity feature. Its mirror, given only the posting,
reaches 0.546, barely above chance. The best representation overall is plain concatenation of the two embeddings at 0.755. In other words, the representation that most enables résumé recognition scores highest.
...
Figure~\ref{fig:ablation} shows the ordering. That evidence pointed us toward a strong conclusion: the benchmark's labels were not relational at all. A direct test refuted it. Decomposing
...
\textbf{The pooled gain, however, does not come from matching.} Moving from the ten
...
Nothing in the construction is specific to résumés. Any benchmark whose instances are (entity, item) pairs with recurring entities admits the same test. Candidate--job, patient--treatment, user--item, student--question, query--document all qualify: condition on the entity, rank within it, compare against the pooled
number. It needs no extra labels and one pass over predictions already computed.
Where the two diverge, the pooled number is measuring something other than the capability it advertises. For algorithmic hiring specifically, this is the
measurement that distinguishes a system that ranks matches from one that ranks
people. This is the target substitution Raghavan et
al.~\cite{raghavan2020hiring} identify as carrying the ethical and legal weight.
...
protocol we adopt, so the naive protocol inflates, modestly. Closing the résumé channel, however, does not collapse performance. It raises it. We expected the
...
ordering instead tracks which side of the pair is novel, and generalising to a new posting is the harder problem. That matters in deployment, where every new requisition presents exactly that condition.
...
ten folds and whose inner three-fold loop selects among eight configurations per model. The budget is identical for every model, since an unequal one compares search effort instead of algorithms.

Tuning rescues no model. The largest gain across all nine is $+0.0077$ AUROC and
three models are marginally worse tuned than untuned, which is what ordinary nested-CV selection noise looks like. The best-to-worst spread is essentially unchanged ($0.0311 \rightarrow 0.0317$); the significance structure, however, \emph{contracts}:
...
Figure~\ref{fig:mltiers} shows placement. Two things drive it. Scoring that must survive a network outage runs on-device, so the evidence sufficiency model ships as exported coefficients and not as a service call, while components whose weights are too large to ship stay in the service. Training
...
Across the nine models, inference latency spans 0.2 to 195.0 $\mu$s per row, a factor of roughly 1{,}000. Serialised size spans 0.9 KB to 63.4 MB, a factor of about 70{,}000. Predictive quality spans 0.031 AUROC.
Figure~\ref{fig:efficiency} shows a frontier that is close to flat, so for an interactive deployment the rational choice is the cheapest model that is not significantly worse, which here is a kilobyte of logistic-regression
...
The first row is a finding, not a limitation of effort. A benchmark whose
...
Contamination audits should normalise before hashing, and should compare entity sets and not only pair sets, since re-pairing the same documents preserves document overlap while destroying pair overlap.
...
Retraining face recognition on a project-scale dataset would be strictly worse than the pretrained model, and would introduce demographic bias that the project has no means to audit. The research question is not the encoder; it is what is done with the
embedding stream downstream. Relatedly, the vendor model pack ships an age and
gender classifier, and the loader specifies an allow-list that never loads it. That is one argument a reviewer can check, not a policy statement.
...
The system originally compared embeddings against an authored similarity threshold of 0.50. The value was arrived at by reasoning, documented honestly, and flagged in its own source comment as uncalibrated. We replaced it with a measured
...
\textbf{This is the article's most consequential single finding.} The prior threshold was not arbitrary --- it was reasoned, documented, and plausible. It was also wrong in a way nobody could have known without measuring, since it would have falsely rejected \textbf{41.4\%} of genuine identity matches. In deployment,
roughly two in five legitimate re-verification checks would have failed. Because the system correctly refuses to treat an unmeasured check as a pass, those candidates would then have accumulated integrity flags for being themselves.
...
\texttt{Unchecked} carries a reason and has \emph{no similarity field at all}. It is not similarity 0.0, not null, not a default. The value does not exist, so no downstream comparison can read a failed measurement as a low-but-real one and
...
Taken alone, each family leaves a gap. Grounding guarantees that claims are the
...
a loop that none closes alone, and Figure~\ref{fig:loop} shows its shape, with the human reviewer at the centre. The loop terminates in a person, not in an
aggregate.
...
Table~\ref{tab:before-after} collects the measured effect of integration. Three things stand out. First, the largest single improvement came not from a model
but from measuring a constant. Second, two rows read ``unmeasured'' before. The property was asserted, not tested. That condition is the one this article argues is most common and least noticed. Third, the last row is a capability
...
Each row converts a claim that requires trust into one that can be checked in seconds by somebody who does not trust us. That is the property we would most like
...
the score; a bias audit of a claim audit examines the distribution of verdicts and the evidence attached to each. That is a smaller and better-posed question.
