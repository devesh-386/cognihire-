...
An evidence-based screening system spans four surfaces, shown in Figure~\ref{fig:layers}. A candidate-facing portal collects consent, résumé, and session media, while an inference service hosts the perception and learned components.
A persistence layer event-sources the session so it can be reconstructed turn by
turn. A recruiter-facing application renders the audit. The separation matters. Every guarantee this article describes is enforced at a boundary between two of these surfaces, not inside any one of them.
...
database trigger. Question plans are never persisted, so a plan cannot be
edited after the fact to match an outcome. It is a small decision that removes a
whole category of post-hoc rationalisation.
...
Neither of the first two rows is hypothetical. Section~\ref{sec:deployment} reports an
...
These instruments share a direction: an automated employment decision must be explicable and contestable. A system architected around a scalar therefore faces a retrofit that its own architecture resists, since the explanatory information was discarded at design time and cannot be recovered afterwards. Figure~\ref{fig:standards} summarises the practices that follow.
...
\emph{perception}. Assigning a task to the wrong family is itself a failure mode. Use a learned model where a rule is correct and the decision becomes unauditable; use a rule where measurement is required and you are left with an unvalidated constant.
...
Naive substring matching is insufficient, however, and Figure~\ref{fig:gate} shows the decision pipeline that replaces it. Beyond verbatim containment the gate
...
never in production''} is split rather than truncated; and clause scoping, so that matching happens inside clause boundaries and never across them.
...
architecture test fails continuous integration if that import appears. The guarantee is a property of the build, not of developer discipline. Table~\ref{tab:grounding-adv} summarises what this buys.
...
No fluency confound & Extraction quality is fidelity to the source, not plausibility of generated text \\
\bottomrule
\end{tabular}
\end{table}
...
Absence of evidence is reported, never omitted \\
\bottomrule
\end{tabular}
\end{table*}
...
\texttt{notExamined}. The four are not orderable, and that is intentional.
...
discarded. Every one contained a period-bearing token: \texttt{Node.js}, \texttt{React.js}, \texttt{asp.net}, \texttt{Python 3.9}. \emph{Zero} occurred without one. The clause splitter treated a period, question mark, or
...
characters never is. Keeping dotted tokens whole makes clauses \emph{longer},
which widens negation scope instead of narrowing it, so the fix cannot weaken the
guard. The measured before and after confirms this: both rejection rates stayed
at exactly 100\%.

The failure mode is worth naming, because the bug was \emph{silent}: nothing crashed, no test failed, and claims simply went missing. Only a corpus-scale evaluation with an explicit true-positive metric could surface it. That is an argument for measuring the properties you claim, and not merely testing the paths you wrote.
