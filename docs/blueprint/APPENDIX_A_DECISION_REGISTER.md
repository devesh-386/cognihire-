...
| ED-85 | Data is organisation-owned, not candidate-owned: `Candidate`, `Role`, `ClaimAudit`, `EnrolmentProfile` are tenant-scoped records under the recruiter org's tenant. A candidate is a subordinate record, not a peer tenant. | Accepted | ARCH | — (implementation-era, backlog CH-0.1.1) | ED-15 | OQ-08, R-05 |

**ED-85 rationale (resolves OQ-08).** OQ-08 was flagged as "the most consequential OQ in the blueprint" — org-owned vs candidate-owned changes every foreign key and the whole tenancy shape, so it was decided first, before any tenant-key migration, per the Roadmap's own Sprint 0 instruction. The decision: **organisation-owned.** This is not an arbitrary pick — it is the only model consistent with RBAC already `[IMPL]` and tested (`lib/core/rbac/permissions.dart`): the recruiter role holds `manageCandidates`, `manageJobRoles`, `viewAllReports`, `manageOrganisation` — an owner's permission shape over records that belong to the org — while the candidate role holds only `viewOwnReports`, `manageOwnProfile`, `configureOwnSession` — a subordinate's read/self-manage shape, never `manageOrganisation` or cross-candidate visibility. A candidate-owned model would have required each candidate to be a tenant boundary of their own, which the existing permission matrix's asymmetry (`viewOwnReports` vs `viewAllReports` as *separate* permissions specifically to prevent this collapse) does not support. Candidate transparency (ED-20) is delivered as a *read view into org-owned data*, not candidate data-ownership. `CandidateRef` (CH-2.2.1) is therefore tenant-scoped to the org, and every downstream FK (`Claim`, `Role`, `ClaimAudit`, `EnrolmentProfile`) carries the org's `tenant_id`, not the candidate's.

| ED-86 | Real `AuthStore` backend is Supabase Auth (`SupabaseAuthStore`), self-hosted at T-MVP | Accepted | SEC | — (implementation-era, backlog CH-1.1.2) | ED-15, ED-85 | OQ-03, OQ-49 |

**ED-86 rationale (resolves OQ-03/OQ-49).** `lib/core/auth/in_memory_auth_store.dart`'s own doc comment already named the successor: *"The hosted `SupabaseAuthStore` is the one that ships."* This decision confirms that choice rather than inventing a new one, and states why it holds up: Supabase Auth is self-hostable (matches T-MVP's single-box philosophy, ED-68 — no new managed dependency required to run the whole stack on one box), is Postgres-native (composes directly with the already-decided Postgres+RLS tenancy model, ED-30, rather than a second identity store needing its own sync), and ships JWT issuance + refresh rotation out of the box (satisfies CH-1.1.3 without a bespoke token service). Role (`UserRole`) and `organisationId` are carried in Supabase's `user_metadata` at sign-up and read back into `Principal` at sign-in — Supabase has no first-class "role" concept of its own, so the existing `AuthStore`/`Principal`/`UserRole` domain types remain the source of truth; Supabase is purely the credential-verification backend behind them.
