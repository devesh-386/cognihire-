...
|---|---|---|---|---|---|
| OQ-08 | What does "organisation" mean — org-owned vs candidate-owned data? | **P0 — RESOLVED** | ARCH | every FK, the whole tenancy shape, tenant-key migration | **Decided: organisation-owned (ED-85).** `Candidate`/`Role`/`ClaimAudit`/`EnrolmentProfile` are tenant-scoped under the recruiter org; a candidate is a subordinate record, not a peer tenant — the only shape consistent with the RBAC matrix already `[IMPL]`. |
...
| OQ-04 | Wire answer scoring for real, or formally delete it? | **P0 — RESOLVED** | ML | two live-prompt rules run on a hardcoded constant | **Decided: delete the concept** (backlog CH-6.1.2 executes it — remove `prompts/scoring_agent.txt` and the `_lastAnswerScore` hardcoding, and strip the two live-prompt rules that depend on it). A rule the system can't satisfy shouldn't ship. |
| OQ-14 | Retention & deletion policy for biometrics + interview data | **P0 — RESOLVED** | COMP | V1-11, K16/K17, BIPA exposure | **Decided:** enrolled face embeddings retained **12 months from last session activity**, or deleted immediately on a candidate deletion request, whichever is first. Interview evidence (claims, telemetry, audit) retained **24 months** from session end for dispute/compliance-window purposes, then crypto-shredded (ED-33/57). Retention beyond the initial consent window requires re-confirmed consent, not silent continuation. |
| OQ-06 | Tamper-proofing for saved audits (chain / signature / server copy) | **P0 — RESOLVED** | SEC | R-08 | **Decided:** extend the existing hash chain to cover the saved `ClaimAudit` JSON itself (today only the embedded event log is chained), and anchor the chain head externally (ED-59) **at every `sessionEnded` event**, not batched — so no session's audit can be silently rewritten in the gap between saves. |
| OQ-17 | Escalation path when identity verification fails mid-session | **P0 — RESOLVED** | PROD | interview state machine | **Decided, formalised as a state machine:** 1st `Unchecked`/`Mismatch` → logged only, session continues. 2nd consecutive → visible warning to candidate + a recruiter-facing flag on the session. 3rd consecutive **`Mismatch`** (not `Unchecked`) → session auto-**paused** pending human review — never auto-failed, never silently continues past 3. Consecutive `Unchecked` alone (camera/service outage — a technical gap, not evidence of misconduct) never auto-pauses; it is surfaced on the audit as "identity coverage degraded" instead. |
...
Several P0s were already **resolved on architectural grounds** (OQ-01, OQ-13, OQ-26) and just needed a delivery decision. The remaining seven — **OQ-08, OQ-04, OQ-06, OQ-14, OQ-17, OQ-18, OQ-81** — were the pre-implementation decision gate (backlog CH-0.1.1–0.1.3). **All seven are now resolved** (see their rows above and ED-85 in Appendix A). The Sprint 0 decision gate is closed; downstream work is no longer blocked on an undecided P0.
